...
We made an abstract class Algorithm with the basics that each algorithm would need, every executer uses this via inheritance.
...
The only algorithm that does not follow this 3 steps is MLFB, it has an extra step Post-tick logic.
...
Our graphs have been smoothed out using statsmodels.nonparametric.smoothers_lowess.
...
SPN, SRT and RR with smaller quantums keep the nTAT much lower than FCFS and this shows that those algorithms favour short processes over longer ones.
...
Smaller quanta improves the short-job responsivness but has an inefficiency for long-running processes while larger quanta has a lower overhead, but weakens the responsiveness advantage RR has over FCFS.
\end{itemize}
